...
/**
 * @description 正则判断字符串是否为密码（8位以上，包含数字、字母、符号）
 */
export function isPassword(val: any) {
    const reg = new RegExp(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[~!@#$%^&*.])[\da-zA-Z~!@#$%^&*.]{8,}$/)
    return !!(reg.test(val))
}

/**
 * @description 判断当前是否为缓存页面
 * @returns boolean
 */
export function isCacheRoute() {
    const route = useRoute();
    const tagsView = useTagsViewStore()
    return typeof route.name === 'string' && tagsView.cachedViews.includes(route.name)
}