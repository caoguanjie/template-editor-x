...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as userApis, {
    get(_, property) {
      return createFunctionalProxy([property], alovaInstance, configMap);
    }
  });
  // define global variable `Apis`
  (globalThis as any).userApis = Apis;
  return Apis;
};
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof userApis
  ? Url extends keyof userApis[Tag]
    ? userApis[Tag][Url] extends (...args: any) => any
      ? Parameters<userApis[Tag][Url]>
      : any
    : any
  : any;
