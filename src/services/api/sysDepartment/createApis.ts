...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as sysApis, {
    get(_, property) {
      return createFunctionalProxy([property], alovaInstance, configMap);
    }
  });
  // define global variable `Apis`
  (globalThis as any).sysApis = Apis;
  return Apis;
};
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof sysApis
  ? Url extends keyof sysApis[Tag]
    ? sysApis[Tag][Url] extends (...args: any) => any
      ? Parameters<sysApis[Tag][Url]>
      : any
    : any
  : any;
